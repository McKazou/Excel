Attribute VB_Name = "Expression_Export_Output_Table"
'From German Macro
...
'----------------------Creating the list of parameters to run---------------------
        ...
        Set rangeToExport = ActiveSheet.ListObjects("output").ListColumns("Export String").Range
        
        For Each target In rangeToExport
            'Target.Select
            ParametersName = target.Value 'Name of the parameter
            NumRow = target.Offset(0, 12).Value 'Row at wich we can find it in the relative sheet
            ...
            'Rajouter un filtre pour les , sur les PCs qui ne sont pas correctement param�tr�
            ...
            Else
                ...
                OutputText(OutputLimit) = target.Value
